...
void loop() {
  ...
  ButtonIsPressed = digitalRead(add_button); // Equals 1 if button being pressed, 0 if not.  
...
void addButtonPressed() {
  ...
  while(digitalRead(add_button) == 1) { /* do nothing until let go of button */ } //AKA: Add Button being held down still
...
void subButtonPressed() {
  ...
  while(digitalRead(subtract_button) == 1) { /* do nothing until let go of button */ } //AKA: Subtract Button being held down still
 
  delay(100); //Delay for 100 milliseconds to prevent spamming of buttons
 
  resetSelector();
 
  while(LED_Selector >= 0 && ButtonIsPressed != 0) // While the LED Selector is greater than zero and the subtract button is 1, then starting from the last LED to the first, do the following:
 
  {
    ...
    else // ELSE (the LED is ON) turn off the LED and set the button state to NOT pressed.
    {
      ...
      ButtonIsPressed = 0; // Reset Button State
    }
  }
}
